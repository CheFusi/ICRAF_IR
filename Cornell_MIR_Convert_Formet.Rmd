...
dnmrc <- df1[,-1] %>% mutate_if(is.character, as.numeric)
df1 <- bind_cols(SSN = df1[,1],dnmrc)
...
pool=df.f_split[sample(nrow(df.f_split), round(0.3*nrow(df.f_split),0)), ]

trial<- df.f_split%>%
  lapply(sample(lapply(df.f_split,nrow)), 30,0)

 lapply(df.f_split,sample(NROW, 1,1))
...
g2 <- ggbiplot::ggbiplot(pcs1, #need to specify which library the command is coming from, gives errors otherwise
              obs.scale = 1,
              var.scale = 1 ,
...
# I changed it to refer to nwraw0 here because that is the data being used to develop the model
#noting also that 'calib' has no significance to calibration here - just kept the same variable name for ease
calib1 <- dim(spec_ref)[1]


#calib <- dim(master)[1]

points(pcss1[1:calib1,1:2], col = "red")

points(pcss1[-c(1:calib1),1:2], col = "blue")

var1 <- round(summary(pcs1)$importance[2,] * 100, 1)

scores1 <- cbind("Calib",as.data.frame(pcs1$x[,1:5])) # get first 5 principal components

names(scores1) <- c("set", colnames(scores1[,-1]))

scores1 <- as.matrix(scores1)

scores1[-c(1:calib1),1] <- "New samples"

scores1 <- as.data.frame(scores1)

write.csv(scores1, file = "./Calib and Pred scores.csv", row.names = FALSE)

scores1 <- read.csv("./Calib and Pred scores.csv")
