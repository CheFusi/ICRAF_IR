...
-   the wavenumbers that undergo cubic spline interpolation are noting the different instruments used, and the spectral ranges they analyze #\* it's unclear what the first number in the series of 3 is
...
#FUSI NOTES: the following are the different instruments used, and the spectral ranges they analyze
#* it's unclear what the first number in the series of 3 is
#* 
...
#spectra based on the range of the instrument 
...
-   The end of the previous code converted the OPUS file into a CSV, so why does the beginning of this code still reference the speclib object which is a python object. I believe this is what leads to the error with the melt(raw, id ="SSN") portion of the code because it doesn't register the raw file as a numeric but instead as some python object
...
#FUSI EDITS:
#read in the data from the CSV, versus from the python object speclib since it imports the data as numeric, versus as a list (of some sort of python object)

raw<-read.table("Raw_spectra.csv", sep= ",", header=TRUE, check.names=FALSE)
#check.names = FALSE prevents R from converting the column names which are in numeric format into character type by inserting an X in front, which introduces issues later

raw<-raw[,-c(2:17)] #removed metadata
...
#FUSI EDITS: initially had commented out the following because they were producing errors 
#but after importing the data from its CSV format, this no longer produced errors. 
